...
flowchart TB
  ...
  subgraph fk["v0.4b — CMR FK ✅"]
    T[TxBean]
    A2[AccountBean]
    T -->|account_id key-fields| A2
  end
  subgraph seq["v0.4d — ID table ✅"]
    N[NextIdBean]
...
| **`@ManyToMany` + join table** | `account`↔`customer`, `CUSTOMER_ACCOUNT_XREF` | **v0.4a** | ✅ `CmpManyToManyToJpa` |
| **CMR `@ManyToOne` / `@OneToMany`** | `tx`→`account`, key-fields in JDBC XML | **v0.4b** | ✅ `CmpForeignKeyToJpa` |
...
| **`NextIdBean` table sequence** | `NEXT_ID` table counters | **v0.4d** | ✅ `NextIdTableToJpa` (retains `getNextId()`; `@GeneratedValue` deferred) |
...
|--------|------------------------|---------------|---------|-------------------|
| **`AccountBean`** | **✅ 3.3** — 7 fields + **v0.4a** M2M | `customers` via `CmpManyToManyToJpa` | Defer `findByCustomerId` | **Done** |
| **`CustomerBean`** | **✅ v0.4c** scalar | Exclude inverse `accounts` M:N in scalar pass | Defer 2 finders | **Done** (scalar) |
| **`TxBean`** | **✅ v0.4c** scalar + **v0.4b** FK | `account` → `@ManyToOne` | Defer | **Done** |
| **`NextIdBean`** | **✅ v0.4d** — 2 fields, table-backed | N/A — ID generation semantics | N/A | **Done** (table entity; sequence migration deferred) |
...
| Role | Assessment |
|------|------------|
| **Pragmatist** | v0.4 complete on all 4 Duke's Bank entities; multi-entity E2E parity verified |
| **Critic** | EJB-QL finders, `LocalNextIdHome` call sites, Customer inverse M:N still deferred — no hidden scope |
| **Suggester** | Next: `@GeneratedValue` / sequence for NextId; automated pattern detection |

**Next gate:** **v0.5** — `EjbQlToNamedQuery`, `RemoveEjbLocalHome` / `LocalNextIdHome` call-site migration ([ADR-007](https://github.com/anchor-migration/migration-hub/blob/main/docs/ADR-007-rewrite-recipes-session-and-cmp-jpa.md)); v0.4 recipes documented under `docs/cmp-*.md` and wired in `demo-dukesbank/scripts/run-e2e-jpa-parity.ps1`.
