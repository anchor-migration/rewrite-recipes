...
- [ ] `@Entity` class with 7 scalar fields; `@Table(name="ACCOUNT")`; `@Column` names match **crosswalk** / `jbosscmp-jdbc.xml`
- [ ] `rewrite-test` before/after on abstract bean file (or new file — spike in 3.3 implementation)
- [ ] Re-export code SSOT + crosswalk: **32 links**, **0 issues** (relationship links may increase later; scalar links must stay green)
- [ ] No `@ManyToMany` / `@OneToMany` on `AccountBean` in 3.3
